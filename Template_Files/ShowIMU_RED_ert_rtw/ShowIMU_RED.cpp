...
void ShowIMU_RED_step(void)
{
  ...
  for (i = 0; i < 3; i++) {
    ShowIMU_RED_B.a[i] = ((b[3 * i + 1] * static_cast<real_T>(b_output[1]) + b[3
      * i] * static_cast<real_T>(b_output[0])) + b[3 * i + 2] *
                          static_cast<real_T>(b_output[2])) +
  ...
  a_idx_0 = ShowIMU_RED_B.a[0] * 2.0 / 32768.0;
  a_idx_1 = ShowIMU_RED_B.a[1] * 2.0 / 32768.0;
  a_idx_2 = ShowIMU_RED_B.a[2] * 2.0 / 32768.0;

  // End of MATLABSystem: '<Root>/LSM9DS1 IMU Sensor'

  // MATLAB Function: '<Root>/MATLAB Function'
  for (i = 0; i < 3; i++) {
    ShowIMU_RED_B.y[i] = 0.0;
    ShowIMU_RED_B.y[i] += static_cast<real_T>(a[i]) * a_idx_0;
    ShowIMU_RED_B.y[i] += static_cast<real_T>(a[i + 3]) * a_idx_1;
    ShowIMU_RED_B.y[i] += static_cast<real_T>(a[i + 6]) * a_idx_2;
  }
...
void ShowIMU_RED_initialize(void)
{
  ...
  // External mode info
  ShowIMU_RED_M->Sizes.checksums[0] = (2446970366U);
  ShowIMU_RED_M->Sizes.checksums[1] = (695111603U);
  ShowIMU_RED_M->Sizes.checksums[2] = (2057495364U);
  ShowIMU_RED_M->Sizes.checksums[3] = (3284420176U);
